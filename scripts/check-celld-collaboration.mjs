...
if (!bindings.some((binding) => binding.name === 'MAP_ROOMS' && binding.class_name === 'MapRoom')) {
  throw new Error('celld MapRoom binding is missing');
}
const migrations = config.migrations ?? [];
if (!migrations.some((migration) => migration.new_sqlite_classes?.includes('MapRoom'))) {
  throw new Error('celld SQLite MapRoom migration is missing');
}
const sources = await Promise.all(
  ['index.ts', 'map-room.ts', 'protocol.ts'].map((file) =>
