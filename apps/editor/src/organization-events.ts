...
export class OrganizationEvents {
  ...
  public connect(signal: AbortSignal): void {
    this.ui.addLayerButton.addEventListener(
      'click',
      () => {
        try {
    ...
    this.ui.layerNameInput.addEventListener(
      'keydown',
      (event) => {
    ...
    this.ui.moveSelectionToLayerButton.addEventListener(
      'click',
      () => {
        try {
    ...
    this.ui.selectLayerButton.addEventListener(
      'click',
      () => {
        const layer = this.app.organization.selectedLayerForPanel();
        if (!layer) return;
    ...
    this.ui.isolateLayerButton.addEventListener(
      'click',
      () => {
        const layer = this.app.organization.selectedLayerForPanel();
    ...
    this.ui.removeLayerButton.addEventListener(
      'click',
      () => {
        const layer = this.app.organization.selectedLayerForPanel();
    ...
    this.ui.layerUpButton.addEventListener(
      'click',
      () => {
        const layer = this.app.organization.selectedLayerForPanel();
    ...
    this.ui.layerDownButton.addEventListener(
      'click',
      () => {
        const layer = this.app.organization.selectedLayerForPanel();
        if (layer?.id) this.state.session.reorderLayer(layer.id, 1);
    ...
    required<HTMLButtonElement>('[data-action="show-all-layers"]').addEventListener(
      'click',
      () => {
    ...
    required<HTMLButtonElement>('[data-action="hide-all-layers"]').addEventListener(
      'click',
      () => {
    ...
    required<HTMLButtonElement>('[data-action="lock-all-layers"]').addEventListener(
      'click',
      () => {
