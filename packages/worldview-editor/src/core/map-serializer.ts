import type { MapDocument, MapFace, Vec3 } from './types.js';
...
function serializeFace(face: MapFace, valve220: boolean): string {
...
export function serializeMap(document: MapDocument): string {
  ...
  for (const entity of document.entities) {
    ...
    for (const brush of entity.brushes) {
      output.push('{');
      for (const face of brush.faces) output.push(serializeFace(face, valve220));
      output.push('}');
    }
    output.push('}');
  }
