...
function encodedTopologyPoint(point: Vec3): string {
  return point.map((component) => Math.round(component / 0.001)).join(',');
}

function topologyHandleKey(kind: EditorTopologyKind, vertices: readonly Vec3[]): string {
  return `${kind}:${vertices.map(encodedTopologyPoint).toSorted().join('|')}`;
}

function translatedTopologyHandle(handle: TopologyHandle, delta: Vec3): TopologyHandle {
...
function topologyHandleVertices(handles: readonly TopologyHandle[]): readonly Vec3[] {
...
function topologyHandleBrushIds(handles: readonly TopologyHandle[]): readonly BrushId[] {
  return [...new Set(handles.flatMap((handle) => handle.brushIds))];
}

function topologyHandleBounds(handles: readonly TopologyHandle[]): Bounds | null {
...
function transformTopologyPoint(point: Vec3, event: EditorTransformDragEvent): Vec3 {
  const relative: Vec3 = [
...
function addScaled(origin: Vec3, direction: Vec3, factor: number): Vec3 {
  return [
...
function dedupeHullPoints(points: readonly Vec3[]): readonly Vec3[] {
  const unique = new Map<string, Vec3>();
  for (const point of points) unique.set(encodedTopologyPoint(point), point);
  return [...unique.values()];
}

function snapPointToPlane(point: Vec3, planePoint: Vec3, normal: Vec3, gridSize: number): Vec3 {
...
function rectangleOnPlane(start: Vec3, end: Vec3, normal: Vec3): readonly Vec3[] {
...
function pointsFormPolygonOnPlane(
...
function snappedDelta(start: Vec3, end: Vec3, gridSize: number): Vec3 {
...
function scalePivot(bounds: Bounds, handle: ScaleHandle, centered: boolean): Vec3 {
...
function snappedScaleFactor(value: number): number {
...
function scaleOverlayVertices(bounds: Bounds, kind: EditorViewportKind): Float32Array {
...
function sweepCapsBounds(caps: readonly (readonly Vec3[])[]): Bounds | null {
...
function sweepScaleHandle(bounds: Bounds): Vec3 {
  const center = boundsCenter(bounds);
  const size: Vec3 = [
    bounds.max[0] - bounds.min[0],
    bounds.max[1] - bounds.min[1],
...
function objectSelectionBounds(
...
function upload(device: GPUDevice, data: Float32Array, usage: GPUBufferUsageFlags): GPUBuffer {
...
function destroyMaterialResource(resource: MaterialResource): void {
...
function gridVertices(kind: EditorViewportKind, requestedSpacing: number): Float32Array {
