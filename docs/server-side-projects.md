...
| ------------------------------------------ | :---: | :----: | :----: |
| Inspect maps, resources, builds, artifacts |  Yes  |  Yes   |  Yes   |
| Join live presence and receive updates     |  Yes  |  Yes   |  Yes   |
| Edit maps, create checkpoints, run builds  |  Yes  |  Yes   |   No   |
| Create maps                                |  Yes  |  Yes   |   No   |
| Manage members and resource mounts         |  Yes  |   No   |   No   |

A valid 4orm session establishes identity, not project access. Non-members receive no project or
map data, and 4orm administrator status does not bypass project membership. Authorization runs
before MapCell, compiler, Artbin, or blob-store work so a rejected request cannot consume those
resources. Signed mutations additionally reject cross-origin browser requests.
...
The home screen presents Local and Hosted sections with equal weight. Hosted browser routes use
`/project/:projectRef` and `/project/:projectRef/map/:mapRef`, where each reference is a
12-character lowercase Nano ID followed by a decorative, non-unique name slug. The ID is the only
lookup and authorization key. Missing or stale slugs remain valid; links generated by the UI use
the current project and map names. New IDs use an unambiguous 32-character alphabet, giving 60 bits
of entropy, and SQLite primary-key enforcement plus bounded regeneration handles collisions. This
is a clean-cut identity contract: legacy UUID browser routes are not retained. Project access
crosses a common workspace port with local-filesystem and remote implementations; core map/session
code remains free of DOM, HTTP, and filesystem concepts.
...
whose only source authority does not exist.
...
and authenticated artifact download are implemented. Public hosted maps use the server build
adapter and do not probe a visitor's loopback interface. Explicitly configured local helpers retain
the existing direct preview workflow.
