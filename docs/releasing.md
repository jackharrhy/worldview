...
checks and the serialized viewer browser suite, checks the package contents, prints the package
name, version, tag, commit, and tarball sizes, and requires the exact version to be typed before
publishing. The complete editor browser suite remains available through `npm run test:browser` but
does not gate a viewer-package release. Prereleases need an explicit tag, such as
