...
Open a hosted project map after signing in through 4orm. The editor joins the assigned room
...
hosted projects; it must not create or join a room.
...
other; then stop and prove subsequent local edits do not enter the room. Separately open `/editor`
without a session, invoke `#collaboration-toggle`, and prove no `/rooms/` request occurs.
...
All public `/rooms/` requests require a valid hosted-map ticket. An arbitrary room ID, stale ticket,
wrong-room ticket, or absent ticket must receive `401`; a viewer ticket must not initialize a room.
